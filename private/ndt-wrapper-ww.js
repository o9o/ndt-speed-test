...
// WebWorker that wraps around NDTjs in ndt-browser-client.js
// Used by NDTWrapper from ndt-wrapper.js
// Do NOT include this from HTML!
...
self.addEventListener('message', function (e) {
  ...
  switch (msg.cmd) {
    case 'start':
      startNDT(msg.hostname, msg.port, msg.protocol, msg.path, msg.update_interval);
      break;
    case 'stop':
      self.close();
      break;
...
function startNDT(hostname, port, protocol, path, update_interval) {
  ...
  var client = new NDTjs(hostname, port, protocol, path, callbacks, update_interval);
  client.startTest();
}
